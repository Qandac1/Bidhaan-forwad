...
Created by: @BIG_FiiSH (adapted/maintained by you)
"""
...
class ForwardBot:
    ...
    async def initialize(self):
        ...
        try:
            users_count = await self.db.get_user_count()
        except Exception:
            users_count = "Unknown"
        ...
        while True:
            try:
                ...
                try:
                    ...
                    print(f"📥 Processing queued message {message_id} (date: {message_date}) from {source_channel.get('title', 'Unknown')} for user {user_id}")
...
"""
    ...
    async def cmd_list(self, event, user_id: int):
        ...
        for i, ch in enumerate(channels, 1):
            mode_icon = "📋" if ch.get('forward_mode', 'copy') == 'copy' else "➡️"
            message += f"**{i}.** {mode_icon} {ch.get('title', 'Unknown')}\n"
            message += f"   Mode: `{ch.get('forward_mode', 'copy')}`\n\n"
                ...
                if await self.db.remove_user_source_channel(user_id, channel_id):
                    ...
                    try:
                        ...
                        if user_client:
                            ...
                            channel_entity = await user_client.get_entity(int(cid))
    ...
    async def cmd_cleanup(self, event, user_id: int):
        ...
        try:
            ...
            if destination:
                dest_id = str(destination.get('channel_id'))
    ...
    async def cmd_mode(self, event, user_id: int, args):
        ...
        try:
            ...
            if 0 <= index < len(channels):
                channel = channels[index]
                if await self.db.set_user_forward_mode(user_id, channel.get('channel_id'), mode):
    ...
    async def cmd_status(self, event, user_id: int):
        ...
        copy_count = sum(1 for ch in channels if ch.get('forward_mode', 'copy') == 'copy')
...
"""
        ...
        try:
            ...
            # Method 2: Check if it's a post link (contains message ID)
                ...
                if '/c/' in channel_input:
                    ...
                    for i, part in enumerate(parts):
                        if part and not part.isdigit() and part != 's' and 't.me' not in part:
    ...
    async def handle_user_channel_message(self, event, user_id: int):
        ...
        try:
            ...
            for ch in channels:
                ...
                db_channel_id = str(ch.get('channel_id'))
                if not db_channel_id.startswith('-'):
                    db_channel_id = f"-100{db_channel_id}"

                if db_channel_id == channel_id or ch.get('channel_id') == str(abs(event.chat_id)):
                    source_channel = ch
                    print(f"✓ Matched source channel: {ch.get('title', 'Unknown')}")
                    break
            ...
            dest_channel_id = destination.get('channel_id')
            ...
            print(f"📤 Adding to queue for destination: {destination.get('title', 'Unknown')} ({dest_channel_id})")
            ...
            if queue_size % 10 == 0 and queue_size > 0:
                ...
                try:
                    stats = await self.db.get_stats()
                    if stats.get('total_forwards', 0) % 50 == 0:
                        await self.log_to_channel(
                            f"**Forwarding Milestone**\n\n"
                            f"📤 Total Forwards: {stats.get('total_forwards')}\n"
                            f"👥 Active Users: {stats.get('total_users')}\n"
    ...
    async def cmd_stats(self, event, user_id: int):
        ...
        try:
            ...
            await event.reply(
                f"📊 **Bot Statistics**\n\n"
                f"👥 Users: {stats.get('total_users', 'Unknown')}\n"
                f"🔐 Logged In: {sessions}\n"
                f"📤 Forwards: {stats.get('total_forwards', 'Unknown')}\n"
                f"🚫 Banned: {stats.get('banned_users', 'Unknown')}\n\n"
                f"✨ Created by @amanbotz"
            )
        except Exception as e:
            await event.reply(f"❌ Error fetching stats: {e}")
    ...
    async def cmd_users(self, event, user_id: int):
        ...
        for i, user in enumerate(users[:20], 1):
            message += f"{i}. {user.get('username', 'Unknown')} (`{user.get('user_id', 'Unknown')}`)\n"
    ...
    async def cmd_ban(self, event, user_id: int, args):
        ...
        try:
            ...
            # If already banned, inform admin
            if await self.db.is_user_banned(ban_user_id):
                await event.reply(f"⚠️ **User `{ban_user_id}` is already banned!**")
                return

            # Attempt to record ban in DB
            try:
                ok = await self.db.ban_user(ban_user_id, reason)
            except AttributeError:
                # If your Database API uses a different method name, adapt here
                ok = False
            except Exception as db_err:
                print(f"⚠️ DB error while banning: {db_err}")
                ok = False
            ...
            if ok:
                ...
                # Disconnect user's active client if present
    ...
    async def cmd_unban(self, event, user_id: int, args):
        ...
        try:
            ...
            try:
                ok = await self.db.unban_user(unban_user_id)
            except AttributeError:
                ok = False
            except Exception as db_err:
                print(f"⚠️ DB error while unbanning: {db_err}")
                ok = False
